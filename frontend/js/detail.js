...
const detailData = async function detail() {
    const data = await axios.get(`http://localhost:3000/qn-a/detail/${listIndex}`, { // QnA 게시글 목록 중에서 id 값을 통해 원하는 상세 페이지에 데이터를 받아옴, data에 할당
        withCredentials: true
    })
    console.log(data.data.data);
    if (data !== null) {
        const date = new Date(data.data.data.createdAt); // 생성 날짜
        ...
        dataNumber.innerHTML = `글번호 : ${data.data.data.id}`; // 글번호 입력
        dataName.innerHTML = `작성자 : ${data.data.data.nick_name}`; // 작성자 입력
        dataDate.innerHTML = `작성일 : ${str}`; // 작성일 입력
        dataTitle.innerHTML = `${data.data.data.qna_title}`; // 제목 입력
        dataContent.innerHTML = `${data.data.data.qna_content}`; // 내용 입력
    }
